...
# Edits: 
# 05 Jan 2025 - Major overhaul on the Infrared sensor. Fixed the 2015 -2016 values by adding the voltage out put to the air temp equation. Remade the file of average infrared down file with new values from 2015-2017. Added an upper limit to infrared up to 540 W/m2. 
# Made a function to read in all the files so the streaming files and the manual download files don't need seperate arguments. 
...
qaqc_fcrmet <- function(data_file = 'https://raw.githubusercontent.com/FLARE-forecast/FCRE-data/fcre-metstation-data/FCRmet.csv',
  ...
  read_met <- function(data_file){
  
  if (is.character(data_file)==T) {
    # Read in the file so we can see how many columns it has
    check_Met <- read_csv(data_file, skip = 4, n_max = 1, col_names=F, show_col_types = F)
    if(length(check_Met)==18){
      # Take out the temp from the net radiometer sensors if it hasn't happened already.
      Met_start <- read_csv(data_file, skip = 4, col_names=F, show_col_types = F)
      Met_start[,17]<-NULL #remove column
      names(Met_start) = c("DateTime","Record", "CR3000Battery_V", "CR3000Panel_Temp_C",
      ...
      Met_start <- read_csv(data_file, skip = 1, col_names=F, show_col_types = F)
  ...
  # read in the files and clean so they all have the same columns and names
  system.time(met_start2 <- lapply(data_file, read_met))
  
  
  # Bind all the QAQC files from above

  # bind the files together 
  Met2 <- dplyr::bind_rows(met_start2)
  
  # convert NaN to NAs in the dataframe
  Met2[sapply(Met2, is.nan)] <- NA
 ...
 if(max(Met$DateTime, na.rm = TRUE)<"2019-04-15 10:19:00"){
  ...
  if(nrow(log)==0){
    ...
    for(i in 1:nrow(log)){
      ...
      if(flag %in% c(1,2)){
        ...
        # drop when taking out the overlap when time reset
        Met <- Met|>
          drop_na(DateTime)
        ...
        if(is.na(maint_adjustment_code)){ ## Just use updated value if no adjustment code is provided
          ...
          #original_value <- Met[Time,maintenance_cols]
          
          Met[Time ,paste0("Flag_",maintenance_cols)] <- flag
          Met[Time, paste0("Note_",maintenance_cols)] <- notes
          Met[Time ,maintenance_cols] <- eval(parse(text = maint_adjustment_code))
  ...
  #             IR_down_sd = sd(InfraredRadiationDown_Average_W_m2, na.rm = T))|>
  #   mutate(across(c(IR_down_average, IR_down_sd), ~ round(., 3)))
  # 
  # write.csv(Ir_down, "FCR_Met_Infrad_DOY_Avg_2018.csv", row.names = F)
  ...
  #If the IR Down is greater than 3SD of DOY and of the absolute difference between the DOY average and the actual value, but the difference is less than 20, then replace with the equation from the manual(5.67*10-8(AirTemp_C_Average+273.15)^4). The equation is not a perfect match because you are missing the output voltage. Double check if it over or under estimates. 
  
  # name of which argument used below
  IR_Dn <- c(which(abs(Met$InfraredRadiationDown_Average_W_m2-Met$IR_down_average)>(3*Met$IR_down_sd) & abs(Met$InfraredRadiationDown_Average_W_m2-Met$IR_down_average)> 20 & !is.na(Met$InfraredRadiationDown_Average_W_m2)))
  
  # Flag and add the note. 
  Met[IR_Dn, "Flag_InfraredRadiationDown_Average_W_m2"] <- 4
  Met[IR_Dn, "Note_InfraredRadiationDown_Average_W_m2"] <- "Greater than 3SD Value corrected with InfRadDn equation as described in metadata"
  Met[IR_Dn, "InfraredRadiationDown_Average_W_m2"] <- 5.67*10^-8*(Met[IR_Dn,"AirTemp_C_Average"]+273.15)^4
  
  # Get rid of columns we don't need
  Met<-Met|>
    select(-c("DOY","IR_down_average","IR_down_sd"))
  
  # Remove the very low infrared radiation up which is less than 150 and set to NA. Check if there needs to be a higher bound like we do for Infrared radiation down. Made the higher limit 540 which is the same as Infrared down
  
  # name of which argument used below
  IR_Up=c(which(Met$InfraredRadiationUp_Average_W_m2<150 & !is.na(Met$InfraredRadiationUp_Average_W_m2)|Met$InfraredRadiationUp_Average_W_m2>540 & !is.na(Met$InfraredRadiationUp_Average_W_m2)))
  
  Met[IR_Up,"Flag_InfraredRadiationUp_Average_W_m2"] <- 4
  Met[IR_Up,"Note_InfraredRadiationUp_Average_W_m2"] <- "Outlier set to NA. Value below 150 W/m2 or above 540 W/m2" 
  Met[IR_Up,"InfraredRadiationUp_Average_W_m2"] <- NA
  
  
  # Remove very high and very low infrared radiation down. 
  ...
  #Replace missing values with estimations from the equation for the infrared radiation down
  
  # Name of which argument to use below
  IR_Dn<-which(Met$Flag_InfraredRadiationDown_Average_W_m2==2)
  
  Met[IR_Dn,"InfraredRadiationDown_Average_W_m2"] <- 5.67*10^-8*(Met[IR_Dn,"AirTemp_C_Average"]+273.15)^4
  Met[IR_Dn,"Note_InfraredRadiationDown_Average_W_m2"] <- "Value corrected with InfRadDn equation as described in metadata using just air temp"
  Met[IR_Dn,"Flag_InfraredRadiationDown_Average_W_m2"] <- 4
  
  # Taking this out in Jan. 2026 because the IR_up would be the same as IR_down
  
  # # Name of which argument to use below
  # IR_Up <- which(Met$Flag_InfraredRadiationUp_Average_W_m2 == 2)
  # 
  # Met[IR_Up, "InfraredRadiationUp_Average_W_m2"] <- 5.67 * 10 ^ -8 * (Met[IR_Up, "AirTemp_C_Average"] +
  #                                                                       273.15) ^ 4
